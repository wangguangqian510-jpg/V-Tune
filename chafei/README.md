...
- GitHub Actions 自动构建 + zsign 第三方自签
...
- 单局 2 分钟
- 敌人 3 种：浮沫（直沉）、焦苦（斜冲抖动）、杂念（死亡分裂 2 小个）
- 玩家 100 心境值，茶筅每 0.55s 自动发射水线（基础穿透 1）
- 灵墨满弹 → 三选一升级（9 项 × 5 阶，升满后兜底精进卡）
- 技能树 3 系：茶种（龙井/普洱/白毫）× 手法（点茶/煎茶/煮茶）× 心境（静/空灵墨/明）
- 本地最高分持久化、暂停/继续、暴击视觉反馈、震动反馈
- 难度曲线：每 60s 敌人强度 ×1.3（120s 时 ×1.69）
